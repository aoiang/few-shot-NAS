...
def main(xargs):
  ...
  xargs.model_config = None
  if xargs.model_config is None:
    model_config = dict2config(
      dict(name='SETN', C=xargs.channel, N=xargs.num_cells, max_nodes=xargs.max_nodes, num_classes=class_num,
           space=search_space, affine=False, track_running_stats=bool(xargs.track_running_stats),
           edge_to_split=xargs.edge_to_split, edge_op=xargs.edge_op), None)
  else:
...
if __name__ == '__main__':
  ...
  parser.add_argument('--runs', type=int, help='current number of runs')
