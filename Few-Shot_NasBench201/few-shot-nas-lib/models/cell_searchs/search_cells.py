...
class NAS201SearchCell(nn.Module):
  ...
  def __init__(self, C_in, C_out, stride, max_nodes, op_names, affine=False, track_running_stats=True, edge_to_split=0,
               edge_op=0, pcdarts_prune_ratio=1):
    ...
    self.split_i, self.split_j = self.split_edge_index(max_nodes=max_nodes)
    for i in range(1, max_nodes):
      for j in range(i):
        if i == self.split_i and j == self.split_j:
          node_str = '{:}<-{:}'.format(i, j)
          if j == 0:
            xlists = [OPS[op_names[self.edge_op]](C_in , C_out, stride, affine, track_running_stats)]
          else:
            xlists = [OPS[op_names[self.edge_op]](C_in, C_out, 1, affine, track_running_stats)]
        else:
          node_str = '{:}<-{:}'.format(i, j)
          if j == 0:
    ...
    for key in self.edge_keys:
      if key == '{:}<-{:}'.format(self.split_i, self.split_j):
        pass
      else:
        self.edge2index[key] = i
        i += 1
    self.num_edges  = len(self.edges)

  def split_edge_index(self, max_nodes):
    sp_edge = 0
    for i in range(1, max_nodes):
      for j in range(i):
        if sp_edge == self.edge_to_split:
          return i, j
        sp_edge += 1
    ...
    for i in range(1, self.max_nodes):
      ...
      for j in range(i):
        ...
        if node_str == '{:}<-{:}'.format(self.split_i, self.split_j):
          inter_nodes.append(sum(layer(nodes[j]) for layer in self.edges[node_str]))
        else:
      ...
      while True: # to avoid select zero for all ops
        ...
        for j in range(i):
          ...
          if node_str != '{:}<-{:}'.format(self.split_i, self.split_j):
            ...
            sops.append(self.edges[node_str][0])
            has_non_zero = True
    ...
    for i in range(1, self.max_nodes):
      ...
      for op_name, j in cur_op_node:
        ...
        if node_str != '{:}<-{:}'.format(self.split_i, self.split_j):
          ...
          # assert op_name == self.op_names[self.edge_op]
          inter_nodes.append(self.edges[node_str][0](nodes[j]))
      nodes.append( sum(inter_nodes) )
    return nodes[-1]
