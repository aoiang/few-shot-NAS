...
class Logger(object):
  ...
  def __init__(self, log_dir, seed, args, create_model_dir=True, use_tf=False):
    """Create a summary writer logging to log_dir."""
    self.seed      = int(seed)
    ...
    self.logger_path = self.log_dir / 'few-shot-NAS_split_edge_ID_{:}_split_operation_{:}.log'.format(args.edge_to_split, op)
  ...
  def path(self, mode, args, transfer_path=None, transfer_model=None):
    valids = ('model', 'best', 'info', 'log')
    if   mode == 'model': return self.model_dir / 'seed-{:}-basic-split-{:}-op-{:}.pth'.format(self.seed, args.edge_to_split,
                                                                                               args.edge_op)
    elif mode == 'best' : return self.model_dir / 'seed-{:}-best-split-{:}-op-{:}.pth'.format(self.seed, args.edge_to_split,
                                                                                               args.edge_op)
    elif mode == 'info' : return self.log_dir / 'seed-{:}-last-info-split-{:}-op-{:}.pth'.format(self.seed, args.edge_to_split,
                                                                                               args.edge_op)
    elif mode == 'transfer' : return Path(transfer_path) / transfer_model
