...
class UniformRandomSupernet(nn.Module):
  ...
  def arch_pool(self):
    ...
    with open('/private/home/linnanwang/yiyang/NAS-Bench-201/nasbench-201-splitted_search/search_pool/' + split_op + '.txt', 'r') as f:
      archs_pool = eval(f.read())

    for arch in archs_pool:
      self.pool.append(Structure(arch))
  ...
  def forward(self, inputs, arch=None):
    ...
    for i, cell in enumerate(self.cells):
      if isinstance(cell, SearchCell):
        if self.training:
          cur_arch = random.choice(self.pool)
          feature = cell.forward_dynamic(feature, cur_arch)
        else:
          if arch is not None:
  ...
  # def valid_forward(self, inputs, arch):
  #   feature = self.stem(inputs)
  #   for i, cell in enumerate(self.cells):
  #     if isinstance(cell, SearchCell):
  #       feature = cell.forward_dynamic(feature, arch)
