    ...
    for i, node_info in enumerate(arch.nodes):
      for op, xin in node_info:
        ...
        if node_str != '{:}<-{:}'.format(self.split_i, self.split_j):
          ...
          # print('logit is', logits[self.edge2index[node_str], op_index])
    return sum(select_logits).item()


  def return_topK(self, K):
    archs_origin = Structure.gen_all(self.op_names, self.max_nodes, False)
    archs = []
    for arch in archs_origin:
      for i, node_info in enumerate(arch.nodes):
        for op, j in node_info:
          node_str = '{:}<-{:}'.format(i + 1, j)
          if node_str == '{:}<-{:}'.format(self.split_i, self.split_j):
            if op == self.op_names[self.edge_op]:
              archs.append(arch)
              break
